...
public class BlockHighlightHandler {
    ...
    private static void spawnBlockParticles(Minecraft mc, BlockPos pos) {
        ...
        for (int i = 0; i < corners.length; i++) {
            ...
            if (useCenterDirection) {
                // FIX: Для портала направление К центру блока
                dirX = centerX - startX;
                dirY = centerY - startY;
                dirZ = centerZ - startZ;
            } else {
                // Для остальных эффектов - случайное направление к соседу
                int[][] neighbors = {
